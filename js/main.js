...
window.calcularTiempoMeta = function () {
    ...
    if (ingresosCard) ingresosCard.textContent = "$" + formatearNumero(resultado.ingresos);
    if (gastosCard) gastosCard.textContent = "$" + formatearNumero(resultado.gastos);
    if (ahorroCard) ahorroCard.textContent = "$" + formatearNumero(resultado.ahorroMensual);
    if (tasaCard) tasaCard.textContent = tasaText(resultado);
};
